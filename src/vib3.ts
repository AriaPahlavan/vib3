...
export class Vib3 {
    ...
    animate(scene = this.scene, allCams = this.cameras) {
        const render = time => {
            ...
            this.animations.forEach(x => x(time * .001, time));
     ...
     * Note: matches its color with the scene color
     */
    enableFogExp2(color = this.backgroundColor || 0xF2F8F7, density = .1) {
     ...
     */
    enableOrbitalControlsFor(camera, target = [0, 0, 0], view = this.primaryView) {
    ...
    private getCurCam(time) {
        if (this.curCamSupplier) {
            const i = this.curCamSupplier(time * .001, time);
